...
Строки дескрипторов содержат производителя, имя продукта зависит от конфигурации NanoVNA, а серийный номер по умолчанию вычисляется по уникальному идентификатору МК (его можно отключить через Config → Mode → *USB DEVICE UID* для совместимости со старыми хостами).
...
За хранение состояния отвечает слой `system/state_manager`:

* При включённом пункте **REMEMBER STATE** (Config → Expert Settings) изменения числа точек, границ свипа, яркости, режима рычага и активного слота калибровки буферизуются в ОЗУ и автоматически записываются во flash спустя короткую паузу после последнего редактирования.
