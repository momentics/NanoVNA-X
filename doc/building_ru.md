...
Проект перестал быть плоским «форком» DiSlord — код распределён по слоям:

| Путь | Назначение |
| --- | --- |
| `include/app`, `src/app` | Прикладные API: служба свипа, консоль, измерительный конвейер, UI-прослойки. |
| `include/services`, `src/services` | Общая инфраструктура (сервис конфигурации, планировщик, шина событий). |
| `include/system`, `src/system` | Низкоуровневые компоненты вроде `state_manager`, отвечающие за персистентность, backup-регистры и отложенные записи. |
| `src/platform`, `boards/nanovna` | Единый BSP для STM32F072/STM32F303, общий с ChibiOS. |
