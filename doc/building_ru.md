...
Проект перестал быть плоским форком — код распределён по слоям:

| Путь | Назначение |
| --- | --- |
| `include/runtime`, `src/runtime` | Прикладные API верхнего уровня: точка входа, фичефлаги, координация свипов и консоль. |
| `include/rf`, `src/rf` | Свип-движок, измерительный конвейер и сопутствующая аналитика. |
| `include/processing`, `src/processing` | DSP и математические помощники для измерений и UI. |
| `include/infra`, `src/infra` | Общая инфраструктура (конфигурация, планировщик, шина событий, менеджер состояния). |
| `include/interfaces`, `src/interfaces` | Чистые порты для CLI, USB, UI и обработчиков данных. |
| `include/ui`, `src/ui` | Отрисовка, контроллеры, адаптеры ввода и встроенные ресурсы (шрифты/иконки). |
| `src/platform`, `boards/STM32F072/STM32F303` | Аппаратно-зависимый код, общий с ChibiOS. |
