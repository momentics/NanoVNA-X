...
- **State manager.** The `src/infra/state/state_manager.c` module sits between the configuration service and application code. It maintains default value tables, restores backup registers, tracks "dirty" state, and schedules deferred flash writes. The UI and Console access it via `include/infra/state/state_manager.h`, centralizing the autosave logic.
...
Russian translation is available in [`doc/building_ru.md`](doc/building_ru.md). The summary below outlines the essential steps for a clean build.
