...
// ' ' Prepends a space for positive signed-numeric types. positive = ' ', negative = '-'. This flag
// is ignored if the + flag exists.
 ...
 *          - <b>q</b> uint32 or uint64 frequency formatting (depend from defined or not
 * CHPRINTF_FREQUENCY_SIZE_64).
 *
...
int chvprintf(BaseSequentialStream* chp, const char* fmt, va_list ap) {
  ...
  while (true) {
    ...
    // ' ' Prepends a space for positive signed-numeric types. positive = ' ', negative = '-'. This
    // flag is ignored if the + flag exists. '0' When the 'width' option is specified, prepends
