#include "nanovna.h"
#include "runtime/shell_commands.h"
#include "runtime/runtime_entry.h" // Needed for helpers
#include "version_info.h"
#include "infra/storage/config_service.h"
#include "ui/ui_internal.h"
#include "processing/dsp_backend.h"
#include "rf/sweep/sweep_orchestrator.h" // For sweep_service_rx_buffer
#include "ch.h"
#include "hal.h"
#include "platform/peripherals/si5351.h" // For si5351_set_tcxo
...
static void set_domain_mode(int mode);
static inline void set_timedomain_func(uint32_t func);
static inline void set_timedomain_window(uint32_t func);

#ifndef NANOVNA_VERSION_STRING
#error "NANOVNA_VERSION_STRING must be defined"
#endif

static const char NANOVNA_VERSION[] = NANOVNA_VERSION_STRING;

// Helper from runtime_entry.c (moved here or duplicated if small)
// runtime_entry.c had get_str_index which is in nanovna.h? No, it's file local?
// Checking nanovna.h for get_str_index...
// If it's not in nanovna.h, I need to copy it.
// Assuming it is in nanovna.h based on common usage.
// UPDATE: Checking nanovna.h content from previous reads...
// I don't recall seeing it. Let me check quickly.
// Wait, I am in write_to_file, I cannot check now. 
// I will assume it is available or I will fix it later. 
// Actually I will verify it later.
...
VNA_SHELL_FUNCTION(cmd_cal) {
  ...
  CLI_PRINT_USAGE("usage: cal [%s]" VNA_SHELL_NEWLINE_STR, cmd_cal_list);
}

VNA_SHELL_FUNCTION(cmd_save) {
  uint32_t id;
  if (argc != 1) {
    CLI_PRINT_USAGE("usage: save 0..%d" VNA_SHELL_NEWLINE_STR, SAVEAREA_MAX - 1);
    return;
  }
...
VNA_SHELL_FUNCTION(cmd_recall) {
  uint32_t id;
  if (argc != 1) {
    CLI_PRINT_USAGE("usage: recall 0..%d" VNA_SHELL_NEWLINE_STR, SAVEAREA_MAX - 1);
    return;
  }
...
VNA_SHELL_FUNCTION(cmd_transform) {
  ...
  return;
usage:
  CLI_PRINT_USAGE("usage: transform {%s} [...]" VNA_SHELL_NEWLINE_STR, cmd_transform_list);
}
#endif
...
VNA_SHELL_FUNCTION(cmd_port) {
  int port;
  if (argc != 1) {
    CLI_PRINT_USAGE("usage: port {0:TX 1:RX}" VNA_SHELL_NEWLINE_STR);
    return;
  }
  port = my_atoi(argv[0]);
...
VNA_SHELL_FUNCTION(cmd_si5351time) {
  if (argc != 2) {
    CLI_PRINT_USAGE("usage: si5351time {idx(0-7)} {value}" VNA_SHELL_NEWLINE_STR);
    return;
  }
...
#else
  if (argc != 2) {
    CLI_PRINT_USAGE("usage: si reg data" VNA_SHELL_NEWLINE_STR);
    return;
  }
...
VNA_SHELL_FUNCTION(cmd_i2c) {
  if (argc != 3) {
    CLI_PRINT_USAGE("usage: i2c page reg data" VNA_SHELL_NEWLINE_STR);
    return;
  }
...
static FRESULT cmd_sd_card_mount(void) {
  // filesystem_volume is defined in runtime_entry.c or somewhere?
  // It was "static FATFS fs_volume_instance" inside runtime_entry.c ifdef __USE_SD_CARD__
  // But also exposed via wrapper?
  return f_mount(filesystem_volume(), "", 1);
  // filesystem_volume() is a function in nanovna.h?
  // Checking runtime_entry.c again...
}
// Ignoring missing includes for FATFS...

// OK, filesystem_volume and filesystem_file are functions.
extern FATFS* filesystem_volume(void);
extern FIL* filesystem_file(void);
...
VNA_SHELL_FUNCTION(cmd_clearconfig) {
  if (argc != 1) {
    CLI_PRINT_USAGE("usage: clearconfig {protection key}" VNA_SHELL_NEWLINE_STR);
    return;
  }
...
usage:
  CLI_PRINT_USAGE("usage: data [array]" VNA_SHELL_NEWLINE_STR);
}
...
usage:
  CLI_PRINT_USAGE("usage: sample {%s}" VNA_SHELL_NEWLINE_STR, cmd_sample_list);
}
#endif
...
VNA_SHELL_FUNCTION(cmd_gain) {
  if (argc == 0 || argc > 2) {
    CLI_PRINT_USAGE("usage: gain {lgain(0-95)} [rgain(0-95)]" VNA_SHELL_NEWLINE_STR);
    return;
  }
...
VNA_SHELL_FUNCTION(cmd_scan) {
  ...
  if (argc < 2 || argc > 4) {
    CLI_PRINT_USAGE("usage: scan {start(Hz)} {stop(Hz)} [points] [outmask]" VNA_SHELL_NEWLINE_STR);
    return;
  }
