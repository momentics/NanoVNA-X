...
- **Уникальный USB-серийный номер по умолчанию.** Каждое устройство теперь объявляется с номером, вычисленным по уникальному идентификатору МК (при необходимости его всё ещё можно отключить в Config → Mode → *USB DEVICE UID*).
