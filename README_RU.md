...
- Исходная кодовая база была насыщена возможностями; NanoVNA‑X сохранил привычную структуру меню, заново включил работу с картой microSD и добавил сценарии сохранения/восстановления калибровок, экспорта S1P/S2P, скриншотов BMP или RLE‑TIFF, дампов прошивки, исполнения сценариев и форматирования карты прямо в приборе.
...
Публичные заголовки вынесены в `include/`, а поддержка плат, скрипты компоновки и стартовый код находятся в каталоге `src/platform/boards/`. Такая структура позволяет использовать единые механизмы измерений и пользовательского интерфейса на обеих аппаратных платформах, ограничиваясь лишь точечными переопределениями.
...
| `src/platform`, `src/platform/boards/*` | Платы и аппаратно-зависимый код, общий с ChibiOS. |
...
Шина событий (`services/event_bus.[ch]`) — это компактный помощник публикации/подписки без динамического выделения памяти. `event_bus_init()` принимает статический массив подписок, опциональные буферы почтового ящика (`msg_t`-очередь и длину) и набор узлов для очереди. Если заданы буферы, `event_bus_publish()` помещает сообщения в почтовый ящик, после чего отдельный поток вызывает `event_bus_dispatch()` и рассылает их подписчикам. В самой NanoVNA-X роль такого «потока» выполняет измерительный (sweep) цикл: перед обработкой UI/сервисного слоя он вычитывает очередь событий, поэтому отложенные USB-команды, изменения конфигурации и уведомления о состоянии хранилища приходят без дополнительных глобальных флагов. Сервис конфигурации сразу фиксирует изменения во flash и публикует `EVENT_STORAGE_UPDATED`, чтобы UI оперативно обновлял индикаторы калибровки. При отсутствии почтового ящика уведомления доставляются синхронно. Обработчики прерываний должны пользоваться `event_bus_publish_from_isr()`, который резервирует узлы очереди с учётом блокировок. Предопределённые темы (`EVENT_SWEEP_STARTED`, `EVENT_SWEEP_COMPLETED`, `EVENT_TOUCH_INPUT`, `EVENT_STORAGE_UPDATED`, `EVENT_CONFIGURATION_CHANGED`, `EVENT_USB_COMMAND_PENDING`) покрывают текущие сценарии; для расширения шины нужно добавить элементы в перечисление `event_bus_topic_t`.
...
* [Developer notes](doc/developer_notes.md) — краткое описание портов и жизненного цикла модулей (EN).
