...
- Выражаются глубокие благодарности @DiSlord за исходный текст прошивки и основы, которые существенно вдохновили и позволили начать работу над этим новым проектом прошивки.
- Проект стремительно развивается и с каждым днём всё больше отходит от исходной реализации.
- Исходная кодовая база была чрезвычайно насыщена функциями; чтобы сделать дальнейшую разработку возможной, подсистема SD временно удалена для снижения сложности и разблокирования ключевой переработки архитектуры.
- Подсистема SD вернётся, когда основные функции будут реализованы в задуманном виде и архитектура станет готовой для её корректной поддержки.

## Улучшения
Начиная с версии @DiSlord, прошивка претерпела значительную переработку архитектуры и стабилизацию. Основное внимание было уделено отзывчивости, удобству автономного использования и производительности на устройствах с ограниченным объемом памяти.
...
*   **Восстановление по таймаутам:** Критически важные подсистемы, включая измерительный движок и шину I2C, защищены таймаутами. "Зависшая" операция больше не блокирует все устройство; вместо этого подсистема пытается корректно восстановить свою работу.
...
*   **"Handshake" USB:** Приглашение командной строки (`ch>`) теперь отправляется *перед* приветственным баннером, что улучшает совместимость с программами для ПК, такими как NanoVNA-Saver.

*   **Сохранение состояния:** Ключевые настройки, такие как полоса ПЧ (IF bandwidth) и электрическая задержка, теперь сохраняются во флеш-память вместе с калибровками.
...
*   **Оптимизация на этапе компоновки (LTO):** Прошивка теперь собирается с включенной LTO, что приводит к уменьшению размера бинарного файла и повышению производительности.
...
*   **Стратегическое использование DMA:** Архитектура DMA была доработана для повышения стабильности измерений. DMA используется для самых требовательных путей данных:
    ...
    *   **Измерительный конвейер (I2S):** Гарантирует доставку сэмплов от кодека без потерь.
