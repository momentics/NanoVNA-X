...
## Улучшения​
* **Шина событий и вспомогательные задачи.** В прошивке используется легковесная шина событий с типизированными темами, через которую UI, измерительный код, обработчики хранения и адаптеры ввода обмениваются уведомлениями без жёстких зависимостей. Дополнительный хелпер-планировщик лишь оборачивает создание и завершение потоков ChibiOS, опираясь на стандартные примитивы RTOS.
* **Фасад измерительного конвейера.** Специальный объект конвейера связывает драйверы платформы и сервис свипа, передавая запросы на выполнение и раскрывая маску активных каналов, чтобы верхние уровни оставались независимыми от аппаратных нюансов плат.
* **Отслеживание состояния свипа.** Сервис свипа ведёт счётчик поколений, управляет индикаторами прогресса UI и LED и координирует цикл измерений и калибровочные флаги.
